...
def test_local_overrides_supersede_committed_layers(tmp_path: Path) -> None:
    ...
    (config_root / "defaults.json").write_text(
        '{"branding": {"university_name": "Base Uni", "logo_path": "base.png"}, "html": {"top_disclaimer": "Base {date}"}, "pdf": {"footer_left": "Base left", "footer_right": "Info {date} {university_name} {year}", "second_page": {"enabled": false, "info_box_text": "Base info", "bottom_disclaimer": "Base disclaimer"}}}',
        encoding="utf-8",
    )
    ...
    assert tweaks["pdf"]["second_page"]["enabled"] is False
    assert tweaks["pdf"]["second_page"]["info_box_text"] == "Base info"
    assert tweaks["pdf"]["second_page"]["bottom_disclaimer"] == "Base disclaimer"


def test_local_overrides_merge_nested_pdf_second_page(tmp_path: Path) -> None:
    plan_path = tmp_path / "CEICAH3707_2026_T1.json"
    plan_path.write_text(
        """
        {
    ...
    (config_root / "defaults.json").write_text(
        '{"pdf": {"second_page": {"enabled": false, "info_box_title": "Base title", "info_box_text": "Base info", "bottom_disclaimer": "Base disclaimer"}}}',
        encoding="utf-8",
    )
    (override_root / "degree" / "3707.json").write_text(
        '{"pdf": {"second_page": {"enabled": true, "info_box_text": "Override info"}}}',
        encoding="utf-8",
    )
    ...
    assert tweaks["pdf"]["second_page"]["enabled"] is True
    assert tweaks["pdf"]["second_page"]["info_box_text"] == "Override info"
    assert tweaks["pdf"]["second_page"]["info_box_title"] == "Base title"
    assert tweaks["pdf"]["second_page"]["bottom_disclaimer"] == "Base disclaimer"
